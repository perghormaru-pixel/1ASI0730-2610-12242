...
public record Address
{
 ...
 public Address(string street, string number, string city, string? stateOrRegion, string postalCode, string country) 
 {
  ...
  if(string.IsNullOrEmpty(number)) throw new ArgumentException("Number cannot be null or empty");
  if(string.IsNullOrEmpty(city)) throw new ArgumentException("City cannot be null or empty");
  if(string.IsNullOrEmpty(postalCode)) throw new ArgumentException("State or city cannot be null or empty");
 ...
 /// <summary>
 /// Returns the string representation of the address in the format: "Street, Number, City, Region, PostalCode, Country"
 /// </summary>
 /// <returns></returns>
